...
namespace katmod
{
	...
	internal class ToxicHoneycomb : PassiveItem
	{
		...
		private void OnPlayerHit(PlayerController player)
		{
			...
			{
				Projectile maidenProjectile = base.Owner.HandleProjectile(9, 5, 14, false, Vector2.zero, true, ((player.PlayerHasActiveSynergy("bzzzzzzzzzzzzzzz") ? 22.5f : 45) * counter), 300);
				...
				if (player.PlayerHasActiveSynergy("Bubeenic"))
				{
					Projectile component = player.HandleProjectile(20f, 12f, 207, false, Vector2.zero, true, UnityEngine.Random.Range(0.0f, 360.0f), 500);
