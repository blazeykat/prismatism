    ...
    class JunkSynthesizer : PassiveItem
    {
        ...
        private void OnEnemyDamaged(float damage, bool fatal, HealthHaver enemy)
        {
            if (enemy.specRigidbody != null && enemy.aiActor != null && base.Owner != null && enemy != null)
            {
                if (fatal)
                {
                    if (Utilities.BasicRandom(base.Owner, 0.97f, 50f))
                    {
                        if (Utilities.BasicRandom(base.Owner, 0.99f, 100f) && !base.Owner.HasPickupID(580) && !base.Owner.HasPickupID(641))
                        {
                            if (Utilities.UncoolRandom(0.05f) && !base.Owner.HasPickupID(580))
                            {
