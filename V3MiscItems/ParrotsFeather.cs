...
namespace katmod
{
    class ParrotsFeather : PlayerItem
	{
		public static void Init()
		{
			...
			Prismatism.Unlocks.Add(item.PickupObjectId, "Enter the abbey of the true gun");
			...
			if (LastOwner != null && LastOwner.CurrentRoom != null)
			{
				...
				if (actor && actor.healthHaver && actor.healthHaver.IsVulnerable && distance < 3 && !IsOnCooldown && !actor.healthHaver.IsBoss)
				{
					if (actor != StoredEnemy || target == null)
					{
						...
						target.GetComponent<tk2dBaseSprite>().PlaceAtPositionByAnchor(StoredEnemy.specRigidbody.UnitTopCenter, tk2dBaseSprite.Anchor.LowerCenter);
